...
function capitalA(s){
    // Add your logic.
    function formatingText(s) {
        let hasNumbers = /\d/.test(s);
        let hasSymbols = /[^a-zA-Z\s]/.test(s);
        let isEndWith=/A$/.test(s);
        
        if (hasNumbers || hasSymbols||!isEndWith) {
            return false;
    }
        return true;
    }
    return formatingText(s);
